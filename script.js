  ...
  // global variable
   ...
   const initiallyDisplayCurrencies = ["USD", "EUR", "GBP", "MMK", "CNY"]
   let baseCurrency
   ...
   //add new currency country to main page
   ...
   //remove exist currency country from main pag
   ...
   function currenciesListClick(event){ 
        if (event.target.classList.contains("close")){ 
            ...
            parentNode.remove();
            addCurrencyList.querySelector(`[data-currency = ${parentNode.id}]`).classList.remove("disable")
   ...
   function currenciesListInputChange(event){ 
        ...
        const newBaseCurrencyAmount = isNaN(event.target.value) ? 0 : Number(event.target.value); // if number is "" give number
   ...
   function populateCurrenciesist() {
        for (let i = 0; i < initiallyDisplayCurrencies.length; i++){ 
            const currency = currencies.find(c => c.abbreviation === initiallyDisplayCurrencies[i]);
